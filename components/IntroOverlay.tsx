  ...
  return (
    ...
    >
      <div className={`flex items-center gap-3 md:gap-5 transition-opacity duration-500 ${isExiting ? 'opacity-0' : 'opacity-100'}`}>
        {/* Star Icon */}
        <span className="text-3xl md:text-6xl text-gray-900 animate-pulse pb-1">✶</span>
        
        {/* Full Name */}
        <h1 className="text-3xl md:text-6xl font-bold tracking-tight text-gray-900 uppercase font-sans">
          Devashish Raikwar
        </h1>
      </div>
      
      <div className={`absolute bottom-12 text-gray-400 text-xs md:text-sm uppercase tracking-[0.3em] animate-bounce transition-opacity duration-500 ${isExiting ? 'opacity-0' : 'opacity-100'}`}>
        Click to Enter
      </div>
    </div>
  );
